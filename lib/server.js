...
module.exports = function () {
  ...
  const public = resolve('/' + root_config.public) // 获取生成路径
  let port = root_config.port || 114 // 获取端口，默认为114

  // 获取请求资源类型
  function getMime(extname, callback) {
    const mime = path.resolve(__dirname + '/mime.json')
    fs.readFile(mime, (err, data) => {
      if (err) throw Error('找不到mime.json文件！')
      //转成JSON
